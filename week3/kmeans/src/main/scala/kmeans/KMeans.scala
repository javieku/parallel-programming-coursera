...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    if (points.isEmpty && means.nonEmpty)
      return GenMap[Point, GenSeq[Point]]((means.head, GenSeq()))
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val accumulator: (Boolean, (Point, Point)) => Boolean = (acc, pair) => acc && (pair._1.squareDistance(pair._2) <= eta)
    (oldMeans zip newMeans).aggregate(true) (accumulator, _&&_)
  }
