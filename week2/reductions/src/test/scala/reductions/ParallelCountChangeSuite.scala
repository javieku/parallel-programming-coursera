...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("parCountChange should invoke the parallel construct 6 times for money == 16, coins == List(1) and moneyThreshold(16)") {
    ...
    check(16, List(1), ParallelCountChange.moneyThreshold(16), 1)
  }

  test("combinedThreshold should return true when the number of coins times money is less than or equal " +
    "to half of the initial number of coins times starting money") {
    val threshold = ParallelCountChange.combinedThreshold(10, List(1,2,5,10))

    assert(threshold(10,List(1)))
  }
}
